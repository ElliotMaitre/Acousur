...
def Stokes2Intensities(year, S0, S1, S2, S3):
    ...
    np.save(year_path / f"{year}_I0_cpu.npy", I0)
    del I0
    gc.collect()

    I1 = S0 + sqrt3_inv * S1 - sqrt3_inv * S2 - sqrt3_inv * S3
    np.save(year_path / f"{year}_I1_cpu.npy", I1)
    del I1
    gc.collect()

    I2 = S0 - sqrt3_inv * S1 - sqrt3_inv * S2 + sqrt3_inv * S3
    np.save(year_path / f"{year}_I2_cpu.npy", I2)
    del I2
    gc.collect()

    I3 = S0 - sqrt3_inv * S1 + sqrt3_inv * S2 - sqrt3_inv * S3
    np.save(year_path / f"{year}_I3_cpu.npy", I3)
    del I3
    gc.collect()
...
def main():
    for year in YEARS:
        ...
        # CVHH, CVVV
        file_CVHH = year_path / "CVHH_cpu.npy"
        file_CVVV = year_path / "CVVV_cpu.npy"
        ...
        # Lmin3, Lmax3
        file_Lmin3 = year_path / "Lmin3_cpu.npy"
        file_Lmax3 = year_path / "Lmax3_cpu.npy"
        if not file_Lmin3.exists() or not file_Lmax3.exists():
            print("Lmin3 and Lmax3")
            Lmin3, Lmax3 = CV_Generalized_Limits([p0, p2])
        ...
        # CVRR, CVgv, CVVN, CVAZ
        files_cv = {
            "CVRR": year_path / "CVRR_cpu.npy",
            "CVgv": year_path / "CVgv_cpu.npy",
            "CVVN": year_path / "CVVN_cpu.npy",
            "CVAZ": year_path / "CVAZ_cpu.npy",
        }
        ...
        files_gen = {
            "CV1": year_path / "CV1_cpu.npy",
            "CVne1": year_path / "CVne1_cpu.npy",
            "CVnemoins1": year_path / "CVnemoins1_cpu.npy",
            "CV_ne0": year_path / "CV_ne0_cpu.npy",
        }
        if not all(f.exists() for f in files_gen.values()):
            print("CV1, ...")
            CV1 = CV_Generalized_equally([p0, p2], 1)
            CVne1 = CV_Generalized_Non_equally([p0, p2], 1)
            CVnemoins1 = CV_Generalized_Non_equally([p0, p2], -1)
        ...
        # Intensities
        I_files = {
            "I0": year_path / f"{year}_I0_cpu.npy",
            "I1": year_path / f"{year}_I1_cpu.npy",
            "I2": year_path / f"{year}_I2_cpu.npy",
            "I3": year_path / f"{year}_I3_cpu.npy",
        }
        ...
        # Lmin, Lmax
        file_Lmin = year_path / "Lmin_cpu.npy"
        file_Lmax = year_path / "Lmax_cpu.npy"
        ...
        # Lmin2, Lmax2
        file_Lmin2 = year_path / "Lmin2_cpu.npy"
        file_Lmax2 = year_path / "Lmax2_cpu.npy"
        if not file_Lmin2.exists() or not file_Lmax2.exists():
            p0 = np.abs(Exx) ** 2
            p1 = np.abs(Eyy) ** 2
            print("Lmin2, Lmax2")
            Lmin2, Lmax2 = CV_Generalized_Limits([p0, p1])
        ...
        # Lmin4, Lmax4
        file_Lmin4 = year_path / "Lmin4_cpu.npy"
        file_Lmax4 = year_path / "Lmax4_cpu.npy"
        if not file_Lmin4.exists() or not file_Lmax4.exists():
            p0 = np.abs(Exx - Eyy) ** 2
            p2 = np.abs(Exx + Eyy) ** 2
            print("Lmin4, Lmax4")
            Lmin4, Lmax4 = CV_Generalized_Limits([p0, p2])
